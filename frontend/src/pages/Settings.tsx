...
export default function Settings() {
  ...
  const [fullResyncLoading, setFullResyncLoading] = useState(false);
  const [fullResyncMsg, setFullResyncMsg] = useState<string | null>(null);
  ...
  const runFullResync = async () => {
    setFullResyncLoading(true);
    setFullResyncMsg(null);
    try {
      const res = await authFetch(`${API}/bank/sync-all`, { method: 'POST', body: JSON.stringify({}) });
      const data = await res.json();
      if (!res.ok) {
        setFullResyncMsg(data?.error || 'Resync failed');
        return;
      }
      const synced = Number(data?.synced || 0);
      const inv = Number(data?.investments_synced || 0);
      setFullResyncMsg(`Done: ${synced} transactions, ${inv} investments synced.`);
    } catch {
      setFullResyncMsg('Resync failed');
    } finally {
      setFullResyncLoading(false);
    }
  };
      ...
      <div className="bg-surface rounded-xl border border-border divide-y divide-border">
        ...
        <div className="px-4 py-3.5">
          <div className="flex items-center gap-3">
            <RefreshCw size={18} className={`text-muted ${fullResyncLoading ? 'animate-spin' : ''}`} />
            <div className="flex-1 min-w-0">
              <div className="text-sm">Full bank resync</div>
              <div className="text-xs text-muted">Refresh balances, transactions, investments, and bank metadata from Powens.</div>
            </div>
            <button
              onClick={runFullResync}
              disabled={fullResyncLoading}
              className="text-xs text-accent-400 hover:text-accent-300 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
              {fullResyncLoading ? 'Running...' : 'Run'}
            </button>
          </div>
          {fullResyncMsg && <div className="text-xs text-muted mt-2">{fullResyncMsg}</div>}
        </div>
        ...
        {isSandbox() && (
          ...
          >
            ...
            <span className="text-sm">Télécharger le PDF showcase</span>
          </a>
        )}
