...
export default function CreditSimulator() {
  ...
  // Reset animation when simulation params change
  useEffect(() => {
    chartAnimated.current = false;
  }, [amount, duration, rate, insuranceRate, differe]);
      ...
      <div className="bg-surface rounded-xl border border-border p-4 mb-4">
        ...
        <ResponsiveContainer width="100%" height={200}>
          <ComposedChart data={chartData} margin={{ top: 0, right: 5, bottom: 0, left: 5 }} barCategoryGap="20%">
            ...
            <Bar yAxisId="bars" dataKey="interest" stackId="a" fill="#f97316" radius={[0, 0, 0, 0]} isAnimationActive={!chartAnimated.current} />
            <Bar yAxisId="bars" dataKey="capital" stackId="a" fill="#22c55e" radius={[3, 3, 0, 0]} isAnimationActive={!chartAnimated.current} onAnimationEnd={() => { chartAnimated.current = true; }} />
            <Line yAxisId="line" type="monotone" dataKey="remaining" stroke="#6b7280" strokeWidth={1.5} dot={false} strokeDasharray="4 4" isAnimationActive={!chartAnimated.current} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
