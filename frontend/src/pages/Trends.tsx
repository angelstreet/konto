...
export default function Trends() {
  ...
  // Reset animation tracking when month range changes
  useEffect(() => {
    animatedCategories.current = new Set();
  }, [monthRange]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 mb-2 h-10">
        <div className="flex items-center gap-1 min-w-0">
          ...
          <h1 className="text-xl font-semibold whitespace-nowrap">{t('nav_trends', 'Tendances')}</h1>
        </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                      <BarChart data={cat.months.map(m => ({ ...m, label: monthLabel(m.month) }))}>
                        ...
                        <Bar
                          ...
                          fill={color}
                          radius={[2, 2, 0, 0]}
                          isAnimationActive={!animatedCategories.current.has(cat.category)}
                          onAnimationEnd={() => { animatedCategories.current.add(cat.category); }}
                        />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
