...
export default function Dashboard() {
  ...
  if (!loading && (!data || data.accountCount === 0)) {
    const connectBank = async () => {
      const res = await authFetch(appendScope(`${API}/bank/connect`), { credentials: "include" });
    ...
    <div>
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Summary blocks */}
          <div className="grid grid-cols-2 sm:flex gap-2 sm:gap-3 mb-2">
            {summaryBlocks.map((block) => {
              const Icon = block.icon;
              return (
                <div key={block.label} className="bg-surface rounded-xl border border-border px-3 py-2.5 sm:px-4 sm:py-3 flex items-center gap-2.5 min-w-0 sm:flex-1">
