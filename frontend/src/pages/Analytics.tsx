...
export default function Analytics() {
  ...
  const pieChartAnimated = useRef(false);
  ...
  // Reset animation flags when period changes so new data animates once
  useEffect(() => {
    barChartAnimated.current = false;
    pieChartAnimated.current = false;
  }, [period]);
    ...
    <div className="space-y-3 max-w-6xl overflow-x-hidden">
      ...
      <div className="flex items-center justify-between gap-2 mb-2 h-10">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h1 className="text-xl font-semibold whitespace-nowrap">{isPerso ? "Budget Perso" : "Budget Pro"}</h1>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <div className="flex items-center gap-1">
            <button onClick={prev} className="p-1.5 rounded-lg text-muted hover:text-white hover:bg-surface-hover min-h-[36px] min-w-[36px] flex items-center justify-center"><ChevronLeft size={16} /></button>
            <span className="text-sm text-white font-medium whitespace-nowrap min-w-[110px] text-center">{moisFr[month - 1]} {year}</span>
            <button onClick={next} className="p-1.5 rounded-lg text-muted hover:text-white hover:bg-surface-hover min-h-[36px] min-w-[36px] flex items-center justify-center"><ChevronRight size={16} /></button>
          </div>
        </div>
      </div>
