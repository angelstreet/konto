...
import PatrimoineSimulator from './pages/PatrimoineSimulator';
...
import BankingScore from './pages/BankingScore';
...
function AppRoutes() {
  ...
  useEffect(() => {
    if (location.pathname === '/crypto' && !keepAlive.crypto) {
      setKeepAlive(prev => ({ ...prev, crypto: true }));
    }
    if (location.pathname === '/actions-fonds' && !keepAlive.actionsFunds) {
      setKeepAlive(prev => ({ ...prev, actionsFunds: true }));
    }
  }, [location.pathname, keepAlive.actionsFunds, keepAlive.crypto]);
  ...
  return (
    <>
      ...
      {keepAlive.actionsFunds && (
        <div className={location.pathname === '/actions-fonds' ? 'block' : 'hidden'}>
          <ActionsFunds />
        </div>
      )}
      ...
      <Routes>
        ...
        <Route path="/banking-score" element={<BankingScore />} />
        <Route path="/crypto" element={null} />
        <Route path="/actions-fonds" element={null} />
        <Route path="/analyse/perso" element={<AnalysisSynthesis />} />
        <Route path="/analyse/pro" element={<AnalysisSynthesis />} />
    ...
    return (
      <div className="min-h-dvh bg-background flex items-center justify-center px-4">
        <div className="w-full max-w-sm text-center">
          ...
          <p className="text-muted text-sm mb-3">Comptabilité simplifiée</p>
          <SignIn
            routing="hash"
            appearance={{
              baseTheme: dark,
              ...
              elements: {
                rootBox: 'w-full',
                card: 'bg-[#1a1a1a] border border-[#333] shadow-none py-4',
                cardBox: 'gap-2',
          ...
          <div className="mt-4 text-center">
            ...
            <button onClick={() => { enableSandbox(); window.location.reload(); }} className="w-full border border-accent-500/30 text-accent-400 hover:bg-accent-500/10 font-medium rounded-lg py-3 transition-colors text-sm">
    ...
    <LogoutProvider value={onLogout}>
      <AmountVisibilityProvider>
        <FilterProvider>
          ...
          {import.meta.env.PROD && <CookieConsent />}
        </FilterProvider>
      </AmountVisibilityProvider>
    </LogoutProvider>
  );
