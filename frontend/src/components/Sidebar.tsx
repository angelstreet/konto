    ...
    children: [
      {
        ...
        icon: User,
        children: [
          { kind: 'leaf', path: '/analyse/perso', icon: LayoutDashboard, labelKey: 'nav_synthesis' },
          ...
          { kind: 'leaf', path: '/ranking', icon: Trophy, labelKey: 'nav_ranking' },
        ],
      },
      {
        kind: 'subgroup',
        ...
        icon: Briefcase,
        children: [
          { kind: 'leaf', path: '/analyse/pro', icon: LayoutDashboard, labelKey: 'nav_synthesis' },
...
const defaultExpandedGroupIndex = navGroups.findIndex(group => group.labelKey === 'nav_group_patrimoine');
...
export default function Sidebar({ onLogout }: Props) {
  ...
  const [openGroup, setOpenGroup] = useState<number | null>(() => {
    ...
    navGroups.forEach((g, gi) => {
      if (!g.labelKey) return;
      if (childContainsActive(g.children, pathname)) active = gi;
    });
    if (active !== null) return active;
    return defaultExpandedGroupIndex >= 0 ? defaultExpandedGroupIndex : null;
  });
  ...
  // Auto-open group/subgroups when navigating — only the active one
  useEffect(() => {
    let nextOpen: number | null = null;
    const activeSubGroups = new Set<string>();
    navGroups.forEach((g, gi) => {
      if (g.labelKey && childContainsActive(g.children, pathname)) {
        nextOpen = gi;
      }
      g.children.forEach((c, ci) => {
        if (c.kind === 'subgroup' && childContainsActive(c.children, pathname)) {
          activeSubGroups.add(`${gi}-${ci}`);
        }
      });
    });
    ...
    } else if (pathname === '/' && defaultExpandedGroupIndex >= 0) {
      setOpenGroup(defaultExpandedGroupIndex);
    }
  }, [pathname]);
  ...
  const renderSubGroup = (sg: NavSubGroup, gi: number, ci: number) => {
    ...
    if (collapsed) {
      // In collapsed mode, show only subgroup entry (no deep items).
      // Clicking it expands sidebar and opens this subgroup.
      return (
        <button
          key={key}
          onClick={() => {
          ...
          title={t(sg.labelKey)}
          className={`w-full flex items-center justify-center min-h-[40px] rounded-lg transition-colors ${
            hasActive ? 'bg-accent-500/10 text-accent-400' : 'text-muted hover:text-white hover:bg-surface-hover'
          }`}
        >
          <sg.icon size={18} strokeWidth={hasActive ? 2 : 1.6} />
        </button>
      );
    }
      ...
      <div className="px-3 py-4 border-b border-border flex items-center justify-between min-h-[56px]">
        {!collapsed && (
          <div className="flex items-center gap-2 min-w-0">
            <img
              src={`${BASE_PATH}/konto-logo.jpg`}
              alt="Konto logo"
              className="w-5 h-5 rounded-sm object-cover border border-border/70 flex-shrink-0"
            />
            <h1 className="text-lg font-bold text-accent-400 tracking-tight truncate">Konto</h1>
            {sandbox && (
              <span className="ml-1 rounded-full border border-amber-400/40 bg-amber-500/15 px-2 py-0.5 text-[10px] font-medium text-amber-300 whitespace-nowrap">
                Demo
              </span>
            )}
          </div>
