...
The backend applies several security layers in order, registered before route handlers:

### CSRF Protection (feat: #763)

Origin header validation on all state-changing requests (POST, PUT, PATCH, DELETE).

- **Allowed origins:** `ALLOWED_ORIGINS` env array (configured from `CORS_ORIGINS`) + `*.vercel.app` preview deployments
- **Skipped paths:** `/api/health`, `/api/bank-callback`, `/api/coinbase-callback` (third-party webhooks that send no Origin)
- **Requests with no Origin:** allowed (same-origin, server-to-server)
- **Rejected:** returns `403 Forbidden: invalid Origin header`

### Rate Limiting (feat: #762)

In-memory sliding-window rate limiter (resets on cold start — fine for serverless):

| Limiter | Limit | Window | Applied to |
|---------|-------|--------|------------|
| General | 100 req/min | 60 s | All `/api/*` routes |
| Auth | 10 req/min | 60 s | `/api/auth/*`, `/api/login`, `/api/register`, `/api/logout` |

On limit exceeded: `429 Too Many Requests` with `Retry-After`, `X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset` headers.

Memory leak prevention: a 60-second cleanup interval purges expired entries when running outside Vercel.

### Middleware Order

```
CORS → CSRF check → Geo-block → IP blacklist → Security headers → Audit log → Rate limiter (general) → Routes → Rate limiter (auth on auth paths)
```
...
| Daily 3 AM | `rotate-turso-token.sh` | Rotate Turso DB token, update `turso.env`, restart PM2 (#810) |
| Daily | `audit_log` cleanup | Purge audit log entries older than 90 days (#807) |
...
All `/api/*` requests are logged to the `audit_log` table:

| Field | Description |
|-------|-------------|
| user_id | Clerk user ID (or null for unauthenticated) |
| ip | Client IP |
| country | CF-IPCountry header value |
| method | HTTP method |
| path | Request path |
| status | Response status code |
| duration_ms | Request duration |

- Admin endpoint: `GET /api/admin/audit-log` (filterable by date, user, country, status)
- Auto-cleanup cron: entries older than 90 days are purged daily

### Geo-Blocking (#808)

Middleware using `CF-IPCountry` header (Cloudflare). Allowed countries:
CH, FR, DE, AT, IT, ES, PT, NL, BE, LU, GB, IE, US, CA, SE, NO, DK, FI, PL, CZ

- Localhost/LAN IPs bypass the check (trusted proxy)
- Blocked attempts logged to `audit_log`

### IP Blacklist (#809)

Auto-detection + manual management:

| Trigger | Threshold | Block Duration |
|---------|-----------|---------------|
| Too many requests | >100 req/min | 1 hour |
| Failed auth | >10/hour | 24 hours |
| 404 flood | >20 in 5 min | 1 hour |

- Admin endpoints: `GET /api/admin/blacklist`, `POST /api/admin/blacklist`
- `ip_blacklist` table with `auto`/`manual` entries and expiry
