...
import { ipBlacklistMiddleware, manualBlacklistHandler } from './middleware/ipBlacklist.js';
...
// --- Helpers to decrypt sensitive connection rows read from DB ---
function decryptBankConn(row: any): any {
  if (!row) return row;
  return { ...row, powens_token: decrypt(row.powens_token), powens_refresh_token: decrypt(row.powens_refresh_token) };
}
function decryptCoinbaseConn(row: any): any {
  if (!row) return row;
  return { ...row, access_token: decrypt(row.access_token), refresh_token: decrypt(row.refresh_token) };
}
function decryptBinanceConn(row: any): any {
  if (!row) return row;
  return { ...row, api_key: decrypt(row.api_key), api_secret: decrypt(row.api_secret) };
}
function decryptDriveConn(row: any): any {
  if (!row) return row;
  return { ...row, access_token: decrypt(row.access_token), refresh_token: decrypt(row.refresh_token) };
}


function calcInvestmentDiff(unitValue: number, unitPrice: number, quantity: number, apiDiff: number, apiDiffPercent: number): { diff: number, diff_percent: number } {
...
function calcInvDiff(unitValue: number, unitPrice: number, quantity: number, apiDiff: number, apiDiffPct: number): [number, number] {
...
const REDIRECT_URI = process.env.POWENS_REDIRECT_URI || process.env.APP_URL ? `${process.env.APP_URL}/api/bank-callback` : 'http://localhost:3003/api/bank-callback';
...
function extractPowensBankMeta(acc: any): { bankId: string | null; bankName: string | null } {
...
async function getUserId(c: any): Promise<number> {
  ...
  // Legacy/API-token mode: use default user — look up via user_profiles
  ...
  // Fallback: still check users.email for backward compat with un-migrated DBs
...
async function refreshPowensToken(connectionId: number): Promise<string | null> {
  ...
  try {
    ...
    // Update connection with new tokens (encrypt before storage)
    await db.execute({
...
// --- Admin: Manual IP Blacklist ---
app.post('/api/admin/blacklist', manualBlacklistHandler);
...
app.post('/api/settings/api-keys/:id/renew', async (c) => {
  const userId = await getUserId(c);
  const id = Number(c.req.param('id'));
...
app.post('/api/estimation/refresh-all', async (c) => {
  try {
    const assetsResult = await db.execute({
...
app.get('/api/report/patrimoine/last', async (c) => {
  const userId = await getUserId(c);
  const result = await db.execute({
...
app.get('/api/report/patrimoine/last/html', async (c) => {
  const userId = await getUserId(c);
  const result = await db.execute({
...
app.get('/api/analysis/categories', async (c) => {
  const userId = await getUserId(c);
  const months = parseInt(c.req.query('months') || '6');
  const usage = c.req.query('usage');
...
app.get('/api/analysis/subscriptions', async (c) => {
  const userId = await getUserId(c);
  const usage = c.req.query('usage');
...
app.get('/api/analysis/summary', async (c) => {
  const userId = await getUserId(c);
  const usage = c.req.query('usage');
...
app.get('/api/analysis/cashflow', async (c) => {
  const userId = await getUserId(c);
  const usage = c.req.query('usage');
...
app.get('/api/analysis/passive-income', async (c) => {
  const userId = await getUserId(c);
  const usage = c.req.query('usage');
...
app.get('/api/v1/analytics/demographics', async (c) => {
  const userId = await getApiUserId(c);
  if (!userId) return c.json({ error: 'Unauthorized' }, 401);
...
app.get('/api/v1/analytics/investments', async (c) => {
  const userId = await getApiUserId(c);
  if (!userId) return c.json({ error: 'Unauthorized' }, 401);
  if (!requireAnalyticsScope(c)) return c.json({ error: 'Analytics scope required' }, 403);
...
app.get('/api/v1/analytics/subscriptions', async (c) => {
  const userId = await getApiUserId(c);
  if (!userId) return c.json({ error: 'Unauthorized' }, 401);
  if (!requireAnalyticsScope(c)) return c.json({ error: 'Analytics scope required' }, 403);
  const from = new Date();
